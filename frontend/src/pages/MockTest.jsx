...
export default function MockTest() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-100 dark:bg-slate-900 select-none">
      ...
      {/* NTA Header */}
      <header className="bg-[#1e40af] text-white px-4 md:px-6 py-3 flex flex-wrap justify-between items-center shadow-md gap-y-2">
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
          <div className="font-bold text-lg md:text-xl tracking-wider">NTA NEET MOCK</div>
          <div className="md:hidden text-right">
            <div className={`text-xl font-mono font-bold ${timeLeft < 600 ? 'text-red-300 animate-pulse' : ''}`}>
              {formatTime(timeLeft)}
            </div>
          </div>
        </div>
        <div className="w-full md:w-auto overflow-x-auto flex gap-2 pb-1 md:pb-0 custom-scrollbar hide-scrollbar">
          {['Physics', 'Chemistry', 'Botany', 'Zoology'].map(sub => (
            <span key={sub} className={`whitespace-nowrap px-3 py-1 text-xs md:text-sm rounded cursor-pointer ${currentQ.subject === sub ? 'bg-white text-[#1e40af] font-bold' : 'hover:bg-blue-700'}`}>
              {sub}
            </span>
          ))}
        </div>
      ...
      {/* NTA Footer Controls */}
      <footer className="bg-slate-200 dark:bg-slate-800 p-3 grid grid-cols-2 md:flex md:justify-between items-center gap-3 shadow-inner">
        <div className="col-span-2 md:col-span-1 flex flex-wrap gap-2 justify-center md:justify-start">
          <button onClick={() => { setMarkedForReview({ ...markedForReview, [currentQIndex]: true }); setCurrentQIndex(Math.min(test.questions.length-1, currentQIndex+1)); }} className="flex-1 md:flex-none px-3 py-2 bg-purple-600 text-white rounded font-semibold text-xs md:text-sm hover:bg-purple-700 whitespace-nowrap text-center">Mark Review</button>
          <button onClick={() => { const newA = {...answers}; delete newA[currentQIndex]; setAnswers(newA); }} className="flex-1 md:flex-none px-3 py-2 bg-white text-slate-700 border border-slate-300 rounded font-semibold text-xs md:text-sm hover:bg-slate-50 whitespace-nowrap text-center">Clear</button>
        </div>
        <div className="col-span-2 md:col-span-1 flex flex-wrap gap-2 justify-center md:justify-end items-center">
          <button onClick={() => setCurrentQIndex(Math.max(0, currentQIndex - 1))} disabled={currentQIndex === 0} className="flex-1 md:flex-none px-3 md:px-6 py-2 bg-white border border-slate-300 rounded font-semibold text-xs md:text-sm disabled:opacity-50 hover:bg-slate-50 text-center">Previous</button>
          <button onClick={() => setCurrentQIndex(Math.min(test.questions.length-1, currentQIndex+1))} className="flex-1 md:flex-none px-3 md:px-8 py-2 bg-green-600 text-white rounded font-semibold text-xs md:text-sm hover:bg-green-700 shadow-md whitespace-nowrap text-center">Save & Next</button>
          <button onClick={() => handleSubmit(false)} className="w-full md:w-auto px-6 py-2 bg-[#1e40af] text-white rounded font-bold text-sm hover:bg-blue-800 shadow-md md:ml-2 text-center mt-2 md:mt-0">Submit</button>
        </div>
      </footer>
    </div>
  );
